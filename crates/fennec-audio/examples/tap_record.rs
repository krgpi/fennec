...
fn main() -> anyhow::Result<()> {
    ...
    use fennec_audio::system_macos::SystemAudioCapture;

    let out_path = "/tmp/tap_test.wav";
    let mut capture = SystemAudioCapture::new(true)?;
    let format = capture.format();
    println!("tap format: {} Hz, {} ch", format.sample_rate, format.channels);
...
fn main() {
    eprintln!("tap_record only works on macOS");
}
